    ...
    def get_bom_lines(self):
        ...
        for rec in self:
            ...
            for sol in sale_order_lines:
                ...
                # while rec.job_order_ids:
            for record in rec.job_order_ids:
                sale_prod_bom = self.env['mrp.bom'].search([
                    ('product_tmpl_id.product_variant_ids', '=', record.product_id.id)])
                print('sale', sale_prod_bom)
                for product in sale_prod_bom:
                    quantity = product.product_qty
                    for line in product.bom_line_ids:
                        ex_job_lines = self.env['mrp.job.order.material.planning'].search(
                            [('job_order_id', '=', rec.id), ('product_id', '=', line.product_id.id)])
                        if not ex_job_lines:
                            rec.job_order_ids |= rec.job_order_ids.new({
