        ...
        if pos_order_data:
            ...
            try:
                for pos_order in pos_order_data:
                    print('validate button date------------',pos_order.get('creation_date'))
                    order_dic = {
                        "InvoiceNumber": "",
        ...
        for order in orders:
            ...
            try:
                if order and order.session_id and order.session_id.config_id and order.session_id.config_id.fbr_authorization:
                    ...
                    print('1111111111111',datetime.strptime(str(order.date_order),"%Y-%m-%d %H:%M:%S").strftime("%Y-%m-%d %H:%M:%S"))
                    order_dic = {
                        "InvoiceNumber": "",
